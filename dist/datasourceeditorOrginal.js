...
function clickedExternalJson(e) {
  ...
  if ($(e.target).data("url")) {
    ...
    url =
      'https://gist.githubusercontent.com/stevage/08f89468f51822ade8d7/raw/ced603a2dd6c4dd8664751bc45915a45f493dcbf/gistfile1.json';
  } else if (targetname == 'ganew') {
    url =
      'https://api.github.com/repos/NICTA/nationalmap/contents/wwwroot/init/ganew.json?ref=ga-datasource';
  } else if (targetname == 'aremi') {
    url =
      'https://api.github.com/repos/NICTA/aremi-natmap/contents/wwwroot/init/aremi.json';
    ...
    url =
      'https://api.github.com/repos/NICTA/nationalmap/contents/wwwroot/init/' +
      targetname + '.json';
  }
...
function populateSources() {
  ...
  function loadedList(j) {
    j.forEach(function(e) {
      if (e.url.match(/\.json/)) {
        ...
        $(appendtarget).append($(
          ...
          "</a>" +
          (e.name.match('00_National_Data_Sets') ?
            ' <ul id="nm-jsons-national"></ul> ' : '') +
          "</li>"
        ));
      }
    ...
    if (appendtarget === "#nm-jsons") {
      $(appendtarget).append(
        '<li>National Data Sets<ul id="nm-jsons-national"></ul></li>');
      appendtarget = '#nm-jsons-national';
      loadDataSourceList(
        'https://api.github.com/repos/NICTA/nationalmap/contents/datasources/00_National_Data_Sets'
      );
    }
  };
  ...
  var source =
    'https://api.github.com/repos/NICTA/nationalmap/contents/datasources';

  // for testing, to reduce wasting Github API calls
  if (false) {
    source =
      'https://gist.githubusercontent.com/stevage/d2aef2fddd7e24e305e5/raw/gistfile1.txt';
  }
